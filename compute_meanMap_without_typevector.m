function compute_meanMap_without_typevector(input_file_names,output_folder_name,frontDistance,width,object_width,object_length)
    logging_func("Compute PIV mean map");
    %% Explanation
    
    %
    % compute_meanMap_without_typevector
    %
    ...
    [meanMap_u_filtered,stdMap_u_filtered] = get_PIV_Data_meanMap_without_typevector(input_file_names,"u_filtered");

    % v_filtered data
    logging_func("Compute mean map of v_filtered data");
    [meanMap_v_filtered,stdMap_v_filtered] = get_PIV_Data_meanMap_without_typevector(input_file_names,"v_filtered");

    % u_original data
    logging_func("Compute mean map of u_original data");
    [meanMap_u_original,stdMap_u_original] = get_PIV_Data_meanMap_without_typevector(input_file_names,"u_original");

    % v_original data
    logging_func("Compute mean map of v_original data");
    [meanMap_v_original,stdMap_v_original] = get_PIV_Data_meanMap_without_typevector(input_file_names,"v_original");

    % velocity_nagnitude
    %logging_func("Load velocity magnitude");
    %[mean_velocity_magnitude,std_velocity_magnitude] = get_PIV_Data_without_typevector(input_file_names,"velocity_magnitude");

    % Vorticity
    logging_func("Compute mean map of vorticity data");
    [meanMap_vorticity,stdMap_vorticity] = get_PIV_Data_meanMap_without_typevector(input_file_names,"vorticity");
