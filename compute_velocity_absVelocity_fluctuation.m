function [velocity_fluctuation_acceleration,velocity_fluctuation_constVelocity,real_x,real_y] = compute_velocity_absVelocity_fluctuation(search_x,search_y,characteristicVelocity_acceleration,characteristicVelocity_constVelocity,PIV_data_filename,Truck_data_filename)
    %% Explanation
    ...
    if (~any(isnan(characteristicVelocity_acceleration)))
        ...
        for ii = 1:length(characteristicVelocity_acceleration)
            ...
            % Compute velocity fluctuation
            PIV_u_matrix =  sqrt(PIV_data.meanMap_u_filtered{ii}.^2 + PIV_data.meanMap_v_filtered{ii}.^2);
