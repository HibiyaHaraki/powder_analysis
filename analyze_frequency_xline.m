clear; close all; clc;
logging_func("Analyze frequency data in x-line");
%% Explanation

% 
% analyze_frequency_xline
%
...
amplitude_record_u_acceleration  = nan(num_search_y,num_search_x);
amplitude_record_v_acceleration  = nan(num_search_y,num_search_x);
amplitude_record_u_constVelocity = nan(num_search_y,num_search_x);
amplitude_record_v_constVelocity = nan(num_search_y,num_search_x);

power_spectrum_u_acceleration  = nan(num_search_y,num_search_x);
power_spectrum_v_acceleration  = nan(num_search_y,num_search_x);
power_spectrum_u_constVelocity = nan(num_search_y,num_search_x);
power_spectrum_v_constVelocity = nan(num_search_y,num_search_x);

for jj = 1:num_search_y
    logging_func(sprintf("Iteration step %d / %d",jj,num_search_y));
    ...
    parfor ii = 1:num_search_x
        ...
        [velocity_u_fluctuation_acceleration,velocity_u_fluctuation_constVelocity,real_x,real_y] = compute_velocity_u_fluctuation(search_x(ii),search_y(jj),characteristicVelocity_acceleration,characteristicVelocity_constVelocity,PIV_data_filename,Truck_data_filename);
        [velocity_v_fluctuation_acceleration,velocity_v_fluctuation_constVelocity,     ~,     ~] = compute_velocity_v_fluctuation(search_x(ii),search_y(jj),characteristicVelocity_acceleration,characteristicVelocity_constVelocity,PIV_data_filename,Truck_data_filename);
...
% Create legend
for ii = 1:num_search_y
    legend_str(ii) = sprintf("y=%.3f mm",search_y(ii)*1000); 
end
...
hold on
for ii = 1:num_search_y
    plot(search_x.*1000,amplitude_record_u_acceleration(ii,:));
end
hold off
grid on
legend(legend_str);
...
hold on
for ii = 1:num_search_y
    plot(search_x.*1000,amplitude_record_v_acceleration(ii,:));
end
hold off
grid on
legend(legend_str);
...
hold on
for ii = 1:num_search_y
    semilogy(search_x.*1000,amplitude_record_u_constVelocity(ii,:));
end
hold off
grid on
legend(legend_str);
...
hold on
for ii = 1:num_search_y
    semilogy(search_x.*1000,amplitude_record_v_constVelocity(ii,:));
end
hold off
grid on
legend(legend_str);
...
hold on
for ii = 1:num_search_y
    plot(search_x.*1000,power_spectrum_u_acceleration(ii,:));
end
hold off
grid on
legend(legend_str);
...
hold on
for ii = 1:num_search_y
    plot(search_x.*1000,power_spectrum_v_acceleration(ii,:));
end
hold off
grid on
legend(legend_str);
...
hold on
for ii = 1:num_search_y
    plot(search_x.*1000,power_spectrum_u_constVelocity(ii,:));
end
hold off
grid on
legend(legend_str);
...
hold on
for ii = 1:num_search_y
    plot(search_x.*1000,power_spectrum_v_constVelocity(ii,:));
end
hold off
grid on
gca.YScale = "log";
