function [mean_data,std_data] = get_PIV_Data_meanMap_without_typevector(input_file_names,data_label)

    %
    % get_PIV_Data_meanMap_without_typevector
    %
    ...
    function S = load_variables(filename)
        S = load(filename,'x','y',data_label);
    end
end